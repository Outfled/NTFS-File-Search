...
/*
* $MFT Resident Attribute Header Layout
...
/*
* $MFT Non-Resident Attribute Header Layout
* https://flatcap.github.io/linux-ntfs/ntfs/concepts/attribute_header.html#:~:text=Overview,attribute%20depends%20on%20two%20things.
*/
...
typedef struct MFT_RESIDENT_ATTRIBUTE_HDR
{
	//MFT_ATTRIBUTE_HEADER		Header;				// Common data structure
	DWORD						AttributeSize;		// Length of the attribute body
	WORD						AttributeOffset;	// Offset to the Attribute
	BYTE						IndexedFlag;		// Indexed flag
	BYTE						Padding;			// Padding
...
typedef struct _ATTRIBUTE_LIST_ENTRY
{
	...
	VCN_t				LowestVcn;
	MFT_FILE_REFERENCE	FileReference;
	WORD				Reserved;
...
typedef struct MFT_DATARUN_ENTRY
{
	UINT64 Length;
	UINT64 Offset;
};
...
{
	// Index Root Header
	DWORD		Type;			// Attribute type (ATTR_TYPE_FILE_NAME: Directory, 0: Index View)
	DWORD		CollationRule;			// Collation rule
	DWORD		IndexEntrySize;				// Size of index block
	BYTE		ClustersPerIndexRecord;		// Clusters per index block (same as BPB?)
...
typedef union MFT_RECORD
{
	PBYTE					pbDataBuffer;
	PMFT_FILE_RECORD_HEADER	pFileRecord;
};
